...
function App() {
  ...
  const [mode, setMode] = useState<'cooling_load' | 'heating_load'>('cooling_load');
  ...
  return (
      <div style={{ position: 'relative', height: '100vh', width: '100vw' }}>
        ...
        <MapView setBuildingHeight={setBuildingHeight} mode={mode} />
        ...
        <div style={{ position: 'absolute', top: 10, left: 10, zIndex: 10, display: 'flex', gap: '10px' }}>
          ...
          <button
              onClick={() => setMode(mode === 'cooling_load' ? 'heating_load' : 'cooling_load')}
              style={{
                padding: '10px 20px',
                fontSize: '16px',
